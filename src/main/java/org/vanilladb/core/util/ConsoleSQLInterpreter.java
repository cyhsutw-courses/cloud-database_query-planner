...
import org.vanilladb.core.remote.jdbc.VanillaDbDriver;
		...
		try {
			...
			while (true) {
				...
				if (cmd.startsWith("exit") || cmd.startsWith("EXIT"))
					break;
				else if (cmd.startsWith("select") || cmd.startsWith("SELECT") || cmd.startsWith("EXPLAIN") || cmd.startsWith("explain"))
					doQuery(cmd);
				else
					doUpdate(cmd);
			}
